...
@mtkbuild ediam_model = EDIAM(;args_ediam...)


# Save parameters in an array
dt = 1    # 1 Year
final_time = 32.0 # Simulate for 30 years

prob = ODEProblem(ediam_model, [], (0.0, final_time), [])
sol_mt = solve(prob,RK4(),dt=dt,adaptive=false)

# Get data
all_variables = SymbolicIndexingInterface.all_variable_symbols(ediam_model)
df_solution = DataFrame(hcat(sol_mt[all_variables]...)', :auto)

rename!(df_solution, string.(all_variables))

df_solution.time = 1980 .+ sol_mt.t

plot(df_solution.time, df_solution[:,"Yce_N(t)"], label = "Advanced region", xlabel = "Tiempo", title = "Fossil energy consumption")
plot!(df_solution.time, df_solution[:,"Yce_S(t)"], label = "Emerging region")




ENERGY_CONSUMPTION_URL_PATH = "https://raw.githubusercontent.com/milocortes/mod_04_concentracion/ccm-2023/datos/climaticos/energy_consumption.csv"

energia_consumo_observado = CSV.read(download(ENERGY_CONSUMPTION_URL_PATH), DataFrame)

#energia_consumo_observado[(energia_consumo_observado.region .== "world") .&& (energia_consumo_observado.energia.=="fosil"),:]

#consumo_energia_fosil = @subset energia_consumo_observado @byrow begin
#    :region == "world"
#    :energia == "fosil"
#end
#consumo_energia_fosil = unstack(consumo_energia_fosil, :year, :economia, :value)

consumo_energia_fosil = @chain energia_consumo_observado begin
    @subset  @byrow begin
        :region == "world"
        :energia == "fosil"
    end
    unstack(_, :year, :economia, :value)
end

plot(df_solution.time, df_solution[:,"Yce_N(t)"], label = "Advanced region", xlabel = "Tiempo", title = "Fossil energy consumption")
plot!(df_solution.time, df_solution[:,"Yce_S(t)"], label = "Emerging region")
plot!(df_solution.time, consumo_energia_fosil[:, "advanced"], label = "Advanced region (Historical)")
plot!(df_solution.time, consumo_energia_fosil[:,"emerging"], label = "Emerging region (Historical)")


### Define loss function


params_symbols = [:γ_re, :k_re, :γ_ce, :k_ce, :η_re, :η_ce, :ν_re, :ν_ce, :labor_growth_S, :labor_growth_N]

function loss_ediam(X)
    args_ediam = Dict(zip(params_symbols, X))
    ...
    MSE_avanzada = mean((df_solution[:,"Yce_N(t)"] - consumo_energia_fosil[:, "advanced"]).^2)
    MSE_emergente = mean((df_solution[:,"Yce_S(t)"] - consumo_energia_fosil[:, "emerging"]).^2)
...
results = optimize(loss_ediam, lower, upper, x0, SAMIN(), Optim.Options(iterations=10^3))


args_ediam = Dict(zip(params_symbols, results.minimizer))
...
### Creación de futuros
alpha_original_param = EDIAM.structure[:parameters][:α][:default]
epsilon_original_param = EDIAM.structure[:parameters][:ε][:default]
stress_scalar = 0.10 
lower_bound_stress = 1 - stress_scalar
upper_bound_stress = 1 + stress_scalar

plan, _ = LHCoptim(100,2,1000)

scaled_plan = scaleLHC(plan,[(lower_bound_stress*alpha_original_param, upper_bound_stress*alpha_original_param),
                             (lower_bound_stress*epsilon_original_param, upper_bound_stress*epsilon_original_param)])



args_ediam = Dict(zip(params_symbols, results.minimizer))

@mtkbuild ediam_model = EDIAM(;α = scaled_plan[1,:][1], ε =scaled_plan[1,:][2],args_ediam...)
